...
from utils.volume_stats import perf_metrics
...
class UNetExperiment:
    ...
    def __init__(self, config, split, dataset):
        ...
        if config.test:
            dirname = self.weights_name.split('.')[0]    
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            data = batch["sample"].to(self.device, dtype=torch.float)
            ...
            # ANSWER: The prediction shape is [BATCH_SIZE, 3, PATCH_SIZE, PATCH_SIZE].
            # The second dimension (3) represents what the model thinks the voxel represents (background, anterior, or posterior hippocampus).
    ...
    def run_test(self):
        ...
        inference_agent = UNetInferenceAgent(parameter_file_path=os.path.join(self.out_dir, param_file_path), model=self.model, device=self.device)
        ...
        for i, x in enumerate(self.test_data):
            ...
            print(f"{x['filename']} Dice {dice:.4f}. {100*(i+1)/len(self.test_data):.2f}% complete")
