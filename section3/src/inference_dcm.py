...
def get_predicted_volumes(pred):
    ...
    volume_tot = volume_ant + volume_post
    return {"anterior": volume_ant, "posterior": volume_post, "total": volume_tot}
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    # TASK: Create the report here and show information that you think would be relevant to clinicians. 
    ...
    draw.multiline_text((10, 90), f"Patient ID: {header.PatientID}\n"
                        ...
                        f"Overlayed masks (white/grey) on top of original saggital, coronal, and axial slices:\n",
                        (255, 255, 255), font=main_font)

    slice_nums = [orig_vol.shape[2]//3, orig_vol.shape[2]//2, orig_vol.shape[2]*3//4]
    
    # show original with prediction mask overtop
    # get slices from original volume
    sag_slice = orig_vol[slice_nums[0], :, :]
    cor_slice = orig_vol[:, slice_nums[1], :]
    axial_slice = orig_vol[:, :, slice_nums[2]]
    
    # get the prediction masks 
    sag_slice_pred = pred_vol[slice_nums[0], :, :]
    cor_slice_pred = pred_vol[:, slice_nums[1], :]
    axial_slice_pred = pred_vol[:, :, slice_nums[2]]
    
    # resize to original volume's size
    ...
    axial_slice_pred = axial_slice_pred[:axial_slice.shape[0],:axial_slice.shape[1]]
    
    # Create a PIL image from array:
    # Numpy array needs to flipped, transposed and normalized to a matrix of values in the range of [0..255]
    nd_img_sag = np.flip((sag_slice/np.max(sag_slice))*0xff).T.astype(np.uint8)
    nd_img_sag_pred = np.flip((sag_slice_pred/np.max(sag_slice_pred))*0xff).T.astype(np.uint8)
    
    nd_img_cor = np.flip((cor_slice/np.max(cor_slice))*0xff).T.astype(np.uint8)
    nd_img_cor_pred = np.flip((cor_slice_pred/np.max(cor_slice_pred))*0xff).T.astype(np.uint8)
    
    nd_img_axial = np.flip((axial_slice/np.max(axial_slice))*0xff).T.astype(np.uint8)
    nd_img_axial_pred = np.flip((axial_slice_pred/np.max(axial_slice_pred))*0xff).T.astype(np.uint8)
    
    mask_sag = nd_img_sag_pred > 0
    nd_img_sag[mask_sag] = nd_img_sag_pred[mask_sag]
    
    
    mask_cor = nd_img_cor_pred > 0
    nd_img_cor[mask_cor] = nd_img_cor_pred[mask_cor]
    
    mask_axial = nd_img_axial_pred > 0
    nd_img_axial[mask_axial] = nd_img_axial_pred[mask_axial]
    
    # This is how you create a PIL image from numpy array
    size = (200,200)
    pil_sag = Image.fromarray(nd_img_sag, mode="L").convert("RGBA").resize(size)
    pil_cor = Image.fromarray(nd_img_cor, mode="L").convert("RGBA").resize(size)
    pil_axial = Image.fromarray(nd_img_axial, mode="L").convert("RGBA").resize(size)
    ...
    pimg.paste(pil_axial, box=(30+2*size[0], 400))
...
if __name__ == "__main__":
    ...
    inference_agent = UNetInferenceAgent(
        device="cpu",
        parameter_file_path=r"/home/workspace/model1.pth")
